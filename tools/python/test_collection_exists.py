#!/usr/bin/env python3
"""Test collection_exists feature across all deployment modes."""
...
def main():
    ...
    console.print("\n[bold]Summary:[/bold]")
    console.print(f"  {'[OK]' if results['embedded'] else '[FAIL]'} Embedded mode: collection_exists method implemented and tested")
    console.print(f"  {'[OK]' if results['standalone'] else '[FAIL]'} Standalone mode: gRPC interface exposed and verified")
    console.print(f"  {'[OK]' if results['distributed'] else '[FAIL]'} Distributed mode: Cluster service implementation complete")
